...
variables
    ...
    begin
        create_Messages:
            message_1 := [type |-> "type1", value |-> 500, receiver |-> 1];
            message_2 := [type |-> "greetMe", name |-> "Tarzan", sender |-> self];
        send1:
            send(message_1, message_1.receiver);
        send2:
            send(message_2, 1);
        receive:
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "dc6204ef" /\ chksum(tla) = "c1ed474a")
VARIABLES queues, pc
...
create_Messages(self) == /\ pc[self] = "create_Messages"
                         /\ message_1' = [message_1 EXCEPT ![self] = [type |-> "type1", value |-> 500, receiver |-> 1]]
...
send1(self) == /\ pc[self] = "send1"
               /\ \/ /\ queues' = [queues EXCEPT ![message_1[self].receiver] =  Append(queues[message_1[self].receiver], message_1[self])]
                  \/ /\ TRUE
                     /\ UNCHANGED queues
               /\ pc' = [pc EXCEPT ![self] = "send2"]
               /\ UNCHANGED << current_message, a, b, message_1, message_2, 
                               response_message >>

send2(self) == /\ pc[self] = "send2"
               /\ \/ /\ queues' = [queues EXCEPT ![1] =  Append(queues[1], message_2[self])]
                  \/ /\ TRUE
                     /\ UNCHANGED queues
...
Client(self) == create_Messages(self) \/ send1(self) \/ send2(self)
                   \/ receive(self) \/ print_res(self)
