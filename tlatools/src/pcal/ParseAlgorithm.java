      ...
      *********************************************************************/
      ...
      try {          
        ...
        if (stmt.getClass().equals( AST.PrintSObj.getClass()))
          ...
          {
              AST.SendCall sendStmt = (AST.SendCall) stmt;
              AST.SendCall result = sendStmt.deepCopy();
              result.col  = sendStmt.col ;
              result.line = sendStmt.line ;
              result.macroCol  = sendStmt.macroCol ;
              result.macroLine = sendStmt.macroLine ;

              result.setOrigin(sendStmt.getOrigin()) ;
              if (macroLine > 0)
              { result.macroLine = macroLine ;
                  result.macroCol  = macroCol ;
              } ;
              result.receiver = sendStmt.receiver.cloneAndNormalize() ;
              result.receiver.substituteForAll(args, params) ;
              // Substitute the channel's name and expression if it's one of the parameters.
              // Probably not used for now as we currently only have a single queue with name, but we do this to be safe.
              int index = params.indexOf(sendStmt.queuesName);
              if( index != -1 ){
                  TLAExpr arg = (TLAExpr) args.elementAt(index) ;
                  TLAToken tok = arg.tokenAt(new IntPair(0,0));
                  if( tok.type == TLAToken.IDENT ) {
                      result.queuesName = tok.string;
                  }
                  // get argument expression
                  TLAExpr newExpr = arg.cloneAndNormalize();
                  ((Vector) newExpr.tokens.elementAt(0)).removeElementAt(0);
                  // add local expression
                  TLAExpr localExpr = sendStmt.receiver.cloneAndNormalize();
                  localExpr.substituteForAll(args, params);
                  int i = 0 ;
                  while (i < localExpr.tokens.size() ) {
                      Vector newline = new Vector() ;
                      Vector line = (Vector) localExpr.tokens.elementAt(i) ;
                      int j = 0 ;
                      while (j < line.size()) {
                  ...
                  result.receiver = newExpr.cloneAndNormalize();
              }
              result.message = sendStmt.message.cloneAndNormalize() ;
              result.message.substituteForAll(args, params) ;
              return result;
          } ;
          ...
          {
              AST.ReceiveCall receiveStmt = (AST.ReceiveCall) stmt;
              AST.ReceiveCall result = receiveStmt.deepCopy();
              result.col  = receiveStmt.col ;
              result.line = receiveStmt.line ;
              result.macroCol  = receiveStmt.macroCol ;
              result.macroLine = receiveStmt.macroLine ;
              result.setOrigin(receiveStmt.getOrigin()) ;
              if (macroLine > 0)
              { result.macroLine = macroLine ;
                  result.macroCol  = macroCol ;
              } ;
              result.queuesName = receiveStmt.queuesName;
              result.targetVarName = receiveStmt.targetVarName;
              result.targetExp = receiveStmt.targetExp.cloneAndNormalize() ;
              result.targetExp.substituteForAll(args, params) ;
              // Substitute the channel's name and expression if it's one of the parameters.
              // Probably not used for now as we currently only have a single queue with name, but we do this to be safe.
              int index = params.indexOf(receiveStmt.targetVarName);
              if( index != -1 ){
                  TLAExpr arg = (TLAExpr) args.elementAt(index) ;
                  TLAToken tok = arg.tokenAt(new IntPair(0,0));
                  if( tok.type == TLAToken.IDENT ) {
                  ...
                  ((Vector) newExpr.tokens.elementAt(0)).removeElementAt(0);
                  // add local expression
                  TLAExpr localExpr = receiveStmt.targetExp.cloneAndNormalize() ;
                  localExpr.substituteForAll(args, params) ;
                  int i = 0 ;
                  while (i < localExpr.tokens.size() ) {
                      Vector newline = new Vector() ;
          ...
          if (stmt.getClass().equals( AST.MaybeFailObj.getClass()))
          { AST.MaybeFail tstmt = (AST.MaybeFail) stmt ;
              AST.MaybeFail result = tstmt.deepCopy();
              result.col  = tstmt.col ;
              result.line = tstmt.line ;
              result.macroCol  = tstmt.macroCol ;
              result.macroLine = tstmt.macroLine ;
              ...
              result.either.ors = new Vector() ;
              int i = 0 ;
              while (i < tstmt.either.ors.size())
              { result.either.ors.addElement(
                      SubstituteInStmtSeq(
                              (Vector) tstmt.either.ors.elementAt(i),
                              args, params, macroLine, macroCol ) ) ;
                  i = i + 1 ;
              } ;
          ...
          if (stmt.getClass().equals( AST.FailObj.getClass()))
          { AST.Fail tstmt = (AST.Fail) stmt ;
              AST.Fail result = tstmt.deepCopy();
              result.col  = tstmt.col ;
              result.line = tstmt.line ;
              result.macroCol  = tstmt.macroCol ;
              result.macroLine = tstmt.macroLine ;
              ...
              result.await.exp = tstmt.await.exp.cloneAndNormalize() ;
              result.await.exp.substituteForAll(args, params) ;
              return result;
          } ;
...
//      }
   ...
   // Erla+ extensions
    ...
    private static AST GetSendCall(String nextTok) throws ParseAlgorithmException {
        /**
         * Parse send(message, receiver) and broadcast(message)
         */

        AST.SendCall result = new AST.SendCall();

        if (nextTok.equals("send")) {
            result.type = PCalErlangConstants.SendType.SEND;
        } else if (nextTok.equals("broadcast")) {
            result.type = PCalErlangConstants.SendType.BROADCAST;
        } else {
            throw new ParseAlgorithmException("Encountered unknown send type: " + nextTok);
        }

        if (result.type == PCalErlangConstants.SendType.SEND) {
            MustGobbleThis("send");
        } else if (result.type == PCalErlangConstants.SendType.BROADCAST){
            MustGobbleThis("broadcast");
        }
        ...
        result.col  = lastTokCol ;
        result.line = lastTokLine ;
        result.message = GetExpr();

        if (result.type == PCalErlangConstants.SendType.SEND) {
            GobbleThis(",");
            result.receiver = GetExpr();
        }
    ...
    private static AST GetReceiveCall() throws ParseAlgorithmException {
        /**
         * Parse receive(message);
         */

        AST.ReceiveCall result = new AST.ReceiveCall();
        PCalLocation beginLoc = GetLastLocationStart();
    ...
    private static AST GetFail() throws ParseAlgorithmException {
       AST.Fail result = new AST.Fail();
       PCalLocation beginLoc = GetLastLocationStart();
       MustGobbleThis("fail");
       MustGobbleThis("(");
       GobbleThis(")");
    ...
    private static AST GetMaybeFail() throws ParseAlgorithmException {
       AST.MaybeFail result = new AST.MaybeFail();
       PCalLocation beginLoc = GetLastLocationStart();
       MustGobbleThis("maybeFail");
       MustGobbleThis("(");
       GobbleThis(")");
       GobbleThis(";");
       result.col = lastTokCol;
